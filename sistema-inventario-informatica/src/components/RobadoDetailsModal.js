...
const RobadoDetailsModal = ({ isOpen, onClose, data }) => {
    if (!isOpen || !data) return null;

    // Filter for stolen items (operativo === 'ROBADO')
    const robadoItems = data.filter(item => item.operativo && item.operativo.trim().toUpperCase() === 'ROBADO');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-[90vw] flex flex-col max-h-[90vh] animate-fadeIn">
                ...
                {/* Header */}
                <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gradient-to-r from-orange-50 to-white rounded-t-xl">
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-orange-100 rounded-lg text-orange-600 shadow-sm">
                            <FaTimesCircle size={24} />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-gray-900 leading-tight">
                                Detalle Equipos Robados
                            </h3>
                            <p className="text-sm text-gray-500 mt-1">
                                {robadoItems.length} equipos robados contabilizados
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto p-0">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-100">
                                ...
                                <tr key={item.id_inventario} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className="flex-shrink-0 h-8 w-8 bg-orange-50 rounded-full flex items-center justify-center text-orange-500 mr-3">
                                                <FaDesktop size={14} />
                                            </div>
                                            <div>
                            ...
                            {robadoItems.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="px-6 py-8 text-center text-gray-500">
                                        No se encontraron equipos robados.
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-xl flex justify-between items-center">
                    <div className="text-sm text-gray-500">
                        * Listado de equipos sustraídos o robados.
                    </div>
                    <div className="flex items-center gap-4">
                        <button
                            onClick={onClose}
...
export default RobadoDetailsModal;
